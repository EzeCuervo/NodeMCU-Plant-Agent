...
#define moisture_topic "place/sensor/moisture"
...
void loop() {
  ...
  if (now - lastMsg > 1000 * 60) {
    ...
    if ( isnan(moist)) {
      ...
      client.publish(moisture_topic, String(moist_readeable).c_str(), true);     
    }
  }
  if (now - lastRecu > 100 ) {
    lastRecu = now;
