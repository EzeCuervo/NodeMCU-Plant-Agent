...
const int moist_sensor_pin = A0; // A0
...
void reconnect(){
  ...
  while (!client.connected()){
    ...
    if (client.connect(client_name, mqtt_user, mqtt_password)){
      ...
      StaticJsonDocument<300> JSONSoilMoist;
      JSONSoilMoist["dev_cla"] = "humidity";
      JSONSoilMoist["name"] = place + " Soil Moist";
      JSONSoilMoist["uniq_id"] = place + "_soilm";
      JSONSoilMoist["unit_of_meas"] = "%";
      JSONSoilMoist["val_tpl"] = "{{ value_json.soilm}}";
      JSONSoilMoist["stat_t"] = "homeassistant/sensor/" + place + "/sensors/state";
      serializeJson(JSONSoilMoist, JSONmessageBuffer);
      client.publish(("homeassistant/sensor/" + place + "/sensorSM/config").c_str(), JSONmessageBuffer, true);
    }
    else
    {
      Serial.print("Error: ");
...
void loop(){
  ...
  if (now - lastMsg > 1000 * 60){
    ...
    if (now - lastSoil > 1000 * 3600 * 5 || moist_readeable < 25){
      ...
      // Oh, nothing to send
      if (isnan(moist)){
        Serial.println("No info, please check moisture sensor!");
      }
      else{
        if (debug){
          Serial.print(" & Soil Moisture: ");
          Serial.println(moist_readeable);
        }
      }
    } else {
      if (debug){
        int minutesSinceLastSoilRead = (now - lastSoil) / 60;
        Serial.print("Minutes since last Moisture read: " + minutesSinceLastSoilRead);
      }
    }
      ...
      jsonBuffer["soilm"] = moist_readeable;
